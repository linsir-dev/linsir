...
public class BaseController {
    ...
    protected List<LabelValue> loadRelatedData(RelatedDataDTO relatedDataDTO, @Nullable String parentId, @Nullable String keyword) {
        ...
        if (V.notEmpty(relatedDataDTO.getParent())) {
            ...
            boolean isDynamicRoot = !parentIdTypeConversion.apply(CommonConstant.TREE_ROOT_ID).equals(rootId) && V.isNoneEmpty(rootId, parentPathColumn);
            if (isDynamicRoot) {
                ...
                queryWrapper.and(query -> {
                    query.likeRight(parentPathColumn, V.isEmpty(parentPath) ? S.valueOf(rootId) : parentPath + CommonConstant.SEPARATOR_COMMA + rootId);
    ...
    protected Serializable getTreeRootId(Class<?> entityClass, Class<?> parentFieldType) {
        return parentFieldType == Long.class ? 0L : CommonConstant.TREE_ROOT_ID;
    }
