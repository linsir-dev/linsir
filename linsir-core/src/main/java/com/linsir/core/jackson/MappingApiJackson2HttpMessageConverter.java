package com.linsir.core.jackson;

import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.linsir.core.jackson.AbstractReadWriteJackson2HttpMessageConverter;
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	public MappingApiJackson2HttpMessageConverter(ObjectMapper objectMapper, LinsirJacksonProperties properties) {
		super(objectMapper, initWriteObjectMapper(objectMapper, properties), initMediaType(properties));
	}

	private static List<MediaType> initMediaType(LinsirJacksonProperties properties) {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper, LinsirJacksonProperties properties) {
		// 拷贝 readObjectMapper
		ObjectMapper writeObjectMapper = readObjectMapper.copy();
		// 大数字 转 字符串
		if (Boolean.TRUE.equals(properties.getBigNumToString())) {
			writeObjectMapper.registerModules(LinsirNumberModule.INSTANCE);
		}
		// null 处理
		if (Boolean.TRUE.equals(properties.getNullToEmpty())) {
			writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new LinsirBeanSerializerModifier()));
			writeObjectMapper.getSerializerProvider().setNullValueSerializer(LinsirBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
		}
